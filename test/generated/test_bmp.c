 ...
 *
 * `example/bmp` is the little-endian worked example and the misaligned one,
 ...
 * format. `example/http` is what that costs: a cap off by one made every real
 ...
 *
 * and their headers are example/bmp/bmp.vectors, which records how to
