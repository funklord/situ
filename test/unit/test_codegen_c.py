...
def test_every_schema_generates_and_compiles(path: Path, tmp_path: Path) -> None:
	...
	backends' versions of it -- which is how `test/schema/edges.situ` came to
...
def test_a_member_past_the_frame_is_refused_not_read(tmp_path: Path) -> None:
	...
	"""
	source, resolved, _ = analyse(ROOT / "example" / "packet" / "packet.situ")
...
def test_every_fuzzable_struct_reaches_the_harness(path: Path) -> None:
	...
	`example/protobuf` was exactly that for as long as `gen-fuzz` has
...
def test_every_prefix_of_a_real_request_answers_honestly(tmp_path: Path) -> None:
	...
	`example/http` rather than a schema written for this, because that is
	...
	header, source = emit((ROOT / "example" / "http" / "http.situ")
...
def test_the_generated_walk_reads_protoc_output(tmp_path: Path) -> None:
	"""The vectors are the ones in test/generated/test_protobuf.c, which came
