"""The poll driver, held to a real socket (0033).

The `--driver poll` artifact is the shipped event loop for rung 6: it owns
...
opens no external process and the peer always terminates. poll(2) is POSIX,
                    ...
                    uint32_t exp_min_datagrams)
{
	...
	const situ_err_t rc = situ_poll_reply_to_run(sp[0], &drive,
...
def test_poll_driver_retransmits_and_expires(tmp_path: Path) -> None:
	"""Generate the poll driver, drive it over a real socket, and prove
	both loop behaviours: a retransmit that then correlates, and an expiry
	after the retry budget."""
	gen = tmp_path / "gen"
	built = subprocess.run(
		[sys.executable, "-m", "situc.cli", "build", str(SCHEMA),
		 "--target", "c", "--layer", "drive", "--driver", "poll",
		 "--out", str(gen)],
	...
	compiled = subprocess.run(
		[COMPILER, *WARNINGS,
		 ...
		 str(gen / "dns_poll.c"), str(RUNTIME / "situ.c"),
		 "-o", str(binary)],
...
def test_poll_is_refused_on_python(tmp_path: Path) -> None:
	"""A driver crosses the backend axis and poll on python is a cell the
	compiler declines. The refusal names both the driver and the target."""
	refused = subprocess.run(
		[sys.executable, "-m", "situc.cli", "build", str(SCHEMA),
		 "--target", "python", "--layer", "drive", "--driver", "poll",
		 "--out", str(tmp_path / "unused")],
		cwd=ROOT, capture_output=True, text=True)
	assert refused.returncode != 0
	assert "poll" in refused.stderr
	assert "python" in refused.stderr


def test_poll_requires_the_drive_layer(tmp_path: Path) -> None:
