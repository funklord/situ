"""The blocking driver, held to a real socket (0033).

The `--driver blocking` artifact is the shipped event loop for rung 6: it owns
...
opens no external process and the peer always terminates. The driver needs
no OS facility beyond a compiler; the refusal tests are pure CLI.
"""
                    ...
                    uint32_t exp_min_datagrams)
{
	...
	const situ_err_t rc = situ_blocking_reply_to_run(sp[0], &drive,
...
@pytest.mark.skipif(COMPILER is None, reason="needs a C compiler")
def test_blocking_driver_retransmits_and_expires(tmp_path: Path) -> None:
	"""Generate the blocking driver, drive it over a real socket, and prove
	both loop behaviours: a retransmit that then correlates, and an expiry
	after the retry budget."""
	gen = tmp_path / "gen"
	built = subprocess.run(
		[sys.executable, "-m", "situc.cli", "build", str(SCHEMA),
		 "--target", "c", "--layer", "drive", "--driver", "blocking",
		 "--out", str(gen)],
	...
	compiled = subprocess.run(
		[COMPILER, *WARNINGS,
		 ...
		 str(gen / "dns_blocking.c"), str(RUNTIME / "situ.c"),
		 "-o", str(binary)],
...
def test_blocking_is_refused_on_python(tmp_path: Path) -> None:
	"""A driver crosses the backend axis and blocking on python is a cell the
	compiler declines. The refusal names both the driver and the target."""
	refused = subprocess.run(
		[sys.executable, "-m", "situc.cli", "build", str(SCHEMA),
		 "--target", "python", "--layer", "drive", "--driver", "blocking",
		 "--out", str(tmp_path / "unused")],
		cwd=ROOT, capture_output=True, text=True)
	assert refused.returncode != 0
	assert "blocking" in refused.stderr
	assert "python" in refused.stderr


def test_blocking_requires_the_drive_layer(tmp_path: Path) -> None:
