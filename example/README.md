...
| [icmp](icmp/) | A field whose meaning depends on an earlier one -- a variant whose arms are all four bytes, so it costs nothing -- and a checksum that covers its own bytes. `[self_as = 0]` is RFC 1071 said in the schema; `test/generated/test_icmp.c` computes it and compares against a number the kernel wrote. |
...
| [mqtt](mqtt/) | The protocol behind most of what is called IoT, and four constructs it puts together that this directory had only apart: a varint that is the frame's own length, a discriminated union over a four-bit type, a run of records ending where the packet does, and three packets whose body is *nothing* -- which is a zero-byte struct, so they stay in the same switch as everything else. Its boundary is PUBLISH's packet identifier, present when the QoS in the *fixed header* says so -- a variant switches on a field it can name, and an arm cannot see the struct the arm is in. Flag-gated presence itself turned out to be four lines and is used three times in the file (26.56). |
| [keystore](keystore/) | A small private record, which is the shape most applications have and the one this directory had none of: four fields, a version byte, and a sealed tail. Written at the floor deliberately -- hand-rolled it is about twenty-five lines of `offset += N`, and the file says so. What it demonstrates is what changes that answer: the version bump re-lays the parameter block, so the map shows every field after it going `Dynamic`, and the sealed interior is `VerifyGated` in all four languages. Asked for by three projects that evaluated situ and could not find their own shape here (26.66). |
| [json](json/) | The mutual cycle, which situ refused until 0054 was amended: a value contains an object, an object contains members, and a member contains a value. Also the first recursion here through a *variant arm*, which is how a tagged tree is written. Read it for what it asked the language for: characters instead of hex codes, `until "a" | "b"` for a scalar that ends at whichever of a set comes first, and `before` for a delimiter that belongs to what follows rather than to what precedes it. `skip` for whitespace between tokens, which belongs to the token it precedes, so a pretty-printed document measures its own length. And `peek` (0057) closed the last of them: a JSON number's first character IS the byte that says it is a number, so the arm began one byte late and `number` could only be a byte run. It is a `scaled i64` now. What remains is the whitespace after the LAST token, which has no member to follow. |
...
| [usb](usb/) | Three bytes, and the whole of decision 0046 in them: a five-bit CRC over an eleven-bit span, so neither the check field nor what it covers is a whole number of bytes. It is also this directory's only `bit_order lsb_first` message -- USB sends a token least significant bit first -- and writing it found three of the six readings assembling a straddling bit field the other way round (26.375), a fault 26.223 had recorded as one the corpus could not reach. Every constant is cited from `include/linux/usb/hcd.h` and `include/linux/usb.h` on the machine this was written on. |
...
```
situc dump-ast example/ipv4/ipv4.situ
situc map      example/ipv4/ipv4.situ
situc map --format=summary example/ntp/ntp.situ
situc map --check example/ipv4/ipv4.situ    # fails if the committed map drifted
situc advise   example/message/message.situ # ranked, costed suggestions
situc gen-checks example/tcp/tcp.situ       # tests that hold the accessors to the map
situc explain  example/message/message.situ message.recs[].value
```
...
```
python3 -m situc.cli map example/ipv4/ipv4.situ > example/ipv4/ipv4.situ.map
```
...
diagnostic, are pinned in `test/unit/test_crypto.py`.
