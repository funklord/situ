...
| [json](json/) | The mutual cycle, which situ refused until 0054 was amended: a value contains an object, an object contains members, and a member contains a value. Also the first recursion here through a *variant arm*, which is how a tagged tree is written. Read it for its boundary as much as its construct -- JSON is a grammar and situ describes layouts, and the two disagree in exactly two places, both named in the file with what they cost together. |
