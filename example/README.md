...
| [json](json/) | The mutual cycle, which situ refused until 0054 was amended: a value contains an object, an object contains members, and a member contains a value. Also the first recursion here through a *variant arm*, which is how a tagged tree is written. Read it for what it asked the language for: characters instead of hex codes, `until "a" | "b"` for a scalar that ends at whichever of a set comes first, and `before` for a delimiter that belongs to what follows rather than to what precedes it. Its remaining boundary is one line -- whitespace, which is a grammar's job. |
