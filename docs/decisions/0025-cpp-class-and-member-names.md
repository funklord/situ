...
it as a constructor with a return type. `tests/schemas/edges.situ` has held
...
compiles` globbed `examples/` and `edges.situ` lives in `tests/schemas/` --
...
`tests/schemas/edges.situ` compiles in C++ for the first time, and the compile
checks in all four backends now read every schema in the repository rather
than `examples/` alone -- one list, in `tests/unit/every_schema.py`, which is
