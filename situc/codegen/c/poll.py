...
def _shared(prefix: str) -> list[str]:
	...
	submit = ident(prefix, "poll", "submit")
	return [
		...
		" uint32_t len)",
		"{",
		f"\t{ident(prefix, 'poll', 'ctx')}_t *sock = ctx;",
		"\tfor (;;) {",
		...
		"}",
		"",
		f"situ_io_t {ident(prefix, 'poll', 'io')}"
		f"({ident(prefix, 'poll', 'ctx')}_t *ctx)",
		"{",
		"\tsitu_io_t io;",
		f"\tio.submit = {submit};",
		"\tio.ctx    = ctx;",
		...
		f"static uint32_t {ident(prefix, 'poll', 'now_ms')}(void)",
		"{",
		"\tstruct timespec ts;",
...
def _header(schema: ast.Schema, resolved: ResolvedSchema, basename: str,
	...
	guard = macro(prefix, basename, "POLL_H")
	lines = [
		...
		f"typedef void (*{ident(prefix, 'poll', 'reply')}_fn)"
		"(uint32_t id, void *user);",
		f"typedef void (*{ident(prefix, 'poll', 'expired')}_fn)"
	...
	for relation, _policy in ready:
		...
		lines += [
			f"situ_err_t {run}(int fd, {drive}_t *drive,",
			f"                 {ident(prefix, 'poll', 'reply')}_fn on_reply,",
			f"                 {ident(prefix, 'poll', 'expired')}_fn"
			" on_expired,",
			"                 void *user);",
			"",
		]
...
def generate(schema: ast.Schema, resolved: ResolvedSchema, basename: str,
	...
	if not ready:
		return {}
	return {
		f"{basename}_poll.h": _header(schema, resolved, basename, prefix,
		                              ready),
		f"{basename}_poll.c": _source(schema, resolved, basename, prefix,
		                              ready),
	}
