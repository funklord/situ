...
class Emitter:
	...
	def element_value(self, struct: ResolvedStruct, run: Placement,
		...
		The run's start is an expression and the rest is a constant, so the
		dynamic half goes into the pointer and the fixed half into the
		subscript `_raw_load` already applies. `element_target` has refused
		a variable stride and a bit-packed member, so there is nothing to
		walk.
		"""
		...
		if member.offset_bits is None:
			return None
		base = self._base_expression(struct, run)
		fixed = index * (run.element_bits // 8) + member.offset_bits // 8
		read = self._load_expression(member.scalar, member,
		                             f"(situ_base(view) + ({base}))", fixed)
		return f"(int64_t){read}"
