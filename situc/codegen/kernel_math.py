...
def crc_width(decl: ast.CodecDecl) -> int | None:
	...
	if width is None or not 1 <= width <= 64:
		return None
	# A NON-reflected code narrower than the byte it consumes needs a
	# different loop: the register has to be left-aligned inside the byte,
	# the table built from a shifted polynomial, and the result shifted back
	# at the end. That is three changes in four backends rather than a wider
	# gate, so it is described correctly and generated by nobody -- which is
	# what returning None here means and what the caller emits a note for.
	#
	# The reflected form has no such trouble at any width: it consumes the
	# byte with right shifts and masks to the width, so `crc5_usb` works and
	# `crc7_mmc` does not. `crc15_can` is wider than a byte and was only ever
	# held out by the multiple-of-eight rule.
	if width < 8 and not kernel.flag("reflect"):
		return None
	return width
