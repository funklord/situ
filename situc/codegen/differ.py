...
def _c(resolved: ResolvedSchema, prefix: str) -> str:
	lines = [
		...
		"\tif (argc != 2) { return 2; }",
		"",
		"\t/* Exactly the message, not a buffer with the message at the",
		"\t * front. A `static uint8_t raw[4096]` holding a 40-byte draw",
		"\t * gives an accessor 4056 bytes of slack to run into, and a read",
		"\t * past the frame that lands there is a valid read of a valid",
		"\t * object -- invisible to a sanitizer as well as to a segfault.",
		"\t * The fuzz harnesses had the same buffer and the same blind",
		"\t * spot; this is the other instrument (26.317). */",
		"\tn   = (uint32_t)(strlen(argv[1]) / 2u);",
		"\traw = (uint8_t *)malloc(n != 0u ? n : 1u);",
...
def _cpp(resolved: ResolvedSchema, prefix: str) -> str:
	lines = [
		...
		'#include "unit.hpp"',
		"",
		"/* The body both entry points share: `main` below, and the",
		" * libFuzzer target at the end of the file. libFuzzer brings its",
		" * own `main`, so the driver cannot be one. */",
		"static int situ_driver(int argc, char **argv)",
		"{",
		"\tstd::uint32_t n = 0;",
		"",
		"\tif (argc != 2) { return 2; }",
		"",
		"\t/* Exactly the message, for the reason the C driver gives: a",
		"\t * fixed buffer with the draw at the front hands every accessor",
		"\t * a tail of slack, and a read past the frame that lands in it",
		"\t * is a valid read of a valid object. This backend is the one",
		"\t * where that matters most -- Rust panics and Python raises,",
		"\t * and C++ reads on quietly (26.317). */",
		"\tstd::vector<std::uint8_t> held(std::strlen(argv[1]) / 2u);",
		"\tstd::uint8_t *raw = held.empty() ? nullptr : held.data();",
	...
	lines.extend([
		...
		"int main(int argc, char **argv)",
		"{",
		"\treturn situ_driver(argc, argv);",
		"}",
		"#endif",
	])

	# The same body as a libFuzzer target, because C++ is the one
	# memory-unsafe backend with no fuzz harness of its own. The generated
	# accessors have had one each since phase 22 and the walker since
	# today; C++ has the property that makes a harness worth having and
	# none of the coverage -- this backend "reads past it silently", in
	# `test_codegen_cpp`'s own words, where Rust panics and Python raises.
	#
	# Under a macro rather than in a second generator: the driver already
	# names every accessor and prints every answer, so a fuzz entry point
	# is the same walk with the bytes coming from somewhere else. Two
	# generators would be two lists of accessors to keep in step, which is
	# the mistake this file exists to avoid one level down.
	#
	# `stdout` goes nowhere under the fuzzer: printing a line per probe at
	# a million executions is the harness spending its budget on write().
	lines.extend([
		"",
		"#ifdef SITU_FUZZ",
		"/* The driver above, over bytes libFuzzer chose. */",
		...
		"\tif (quiet == nullptr) {",
		"\t\tquiet = std::getenv(\"SITU_FUZZ_LOUD\");",
		"\t\tif (quiet == nullptr && std::freopen(\"/dev/null\", \"w\",",
		"\t\t                                    stdout) == nullptr) {",
		"\t\t\treturn 0;",
		"\t\t}",
		"\t\tquiet = const_cast<char *>(\"\");",
		"\t}",
		...
		"",
		"\t\targv[0] = name;",
		"\t\targv[1] = const_cast<char *>(hex.c_str());",
		"\t\targv[2] = nullptr;",
		"\t\t(void)situ_driver(2, argv);",
		"\t}",
		"\treturn 0;",
		"}",
		"#endif /* SITU_FUZZ */",
