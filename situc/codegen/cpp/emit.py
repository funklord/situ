...
class Emitter:
	...
	def _gate(self, struct: ResolvedStruct, region: Placement) -> list[str]:
		...
		inside  = [entry for entry in struct.entries
		           ...
		           # `_is_run`, not the two spellings it knew: a member sized
		           # by arithmetic has neither, so it appeared here *and* in
		           # `rest` below once that learned the third -- two accessors
		           # of one name (26.188).
		           and not _is_run(entry.placement)]

		rest    = [entry.placement for entry in struct.entries
		           if entry.placement.sealed_by == region.name
		           and entry.placement.kind == "field"
		           # `data_sized`: see the note in the Python emitter. A
		           # length written as arithmetic over a field has neither an
		           # `array_count` nor a `sized_by` (26.188).
		           and _is_run(entry.placement)]
	...
	def _in_gate(self, struct: ResolvedStruct, text: str) -> str:
		...
		stems = {bare_name(local_name(struct, held))
	...
	def _chain_body(self, struct: ResolvedStruct,
		...
		for other in parts:
			...
			lines.append(f"\t\tat = situ_advance_u32(at, {length}, raw_.limit);")
	...
	def _delimited(self, struct: ResolvedStruct,
		...
		room  = f"situ_remaining_u32(raw_.limit, {start})"
		...
		# had this.
		room_at  = "situ_remaining_u32(raw_.limit, at)"
	...
	def _repeat_while(self, struct: ResolvedStruct,
		...
		def walk_from(base: str, depth: str | None = None) -> list[str]:
			...
			"\t\tstd::uint32_t n  = 0;",
			"",
			f"\t\twhile (at < raw_.limit{cap}) {{",
			"\t\t\tsitu_view_t raw;",
			"\t\t\tif (situ_view_sub(this->raw(), at, raw_.limit - at, &raw)"
			" != SITU_OK) {",
			"\t\t\t\tbreak;",
			"\t\t\t}",
	...
	def _record_run(self, struct: ResolvedStruct,
		...
		return [
			...
			*from_,
			...
			f"\t\tif (at + {len(delim)}u <= raw_.limit) {{",
			f"\t\t\tat += {len(delim)}u;",
			"\t\t}",
			"\t\treturn at - start;",
			"\t}",
	...
	def _required(self, struct: ResolvedStruct) -> list[str]:
		...
		for placement in variable:
			...
			name_of = bare_name(local_name(struct, placement))
			steps.extend([
				"",
	...
	def _content_length_expression(self, struct: ResolvedStruct,
		...
		if placement.varint is not None:
			if not self._reads_varint(placement):
				return None
			return f"{bare_name(local_name(struct, placement))}_len()"
		...
		if placement.sized_by == "remaining":
			start = self._offset_expression(struct, placement)
			return None if start is None else f"(raw_.limit - ({start}))"
	...
	def _count_expression(self, struct: ResolvedStruct,
		...
		if driver.placement.varint is not None:
			if not self._reads_varint(driver.placement):
				return None
			name = bare_name(local_name(struct, driver.placement))
	...
	def _index_element_extent(self, element: ResolvedStruct) -> list[str]:
		...
		return [
			...
			"\t\t\treturn ::situ::rt::err::bounds;",
			"\t\t}",
			"\t\tif (situ_view_sub(this->raw(), start, raw_.limit - start, &raw)",
			"\t\t\t\t!= SITU_OK) {",
			"\t\t\treturn ::situ::rt::err::bounds;",
			"\t\t}",
			f"\t\tif (situ_view_sub(this->raw(), start,",
	...
	def _lead_methods(self, struct: ResolvedStruct,
		...
		content = (str(fixed_span_bits(placement) // BITS_PER_BYTE) + "u"
	...
	def _offsets(self, struct: ResolvedStruct) -> list[str]:
		...
		fields = [f"\t\tstd::uint32_t {bare_name(local_name(struct, held))};"
		          for held in dynamic]
	...
	def _variable_elements(self, struct: ResolvedStruct, placement: Placement,
		...
		return [
			...
			"\t{",
			f"\t\treturn situ_min_u32({length},",
			f"\t\t\tsitu_remaining_u32(raw_.limit, {name}_offset())) / {width}u;",
			"\t}",
	...
	def _validate(self, struct: ResolvedStruct) -> list[str]:
		...
		if not checks and not self._refuse_checks(struct) \
