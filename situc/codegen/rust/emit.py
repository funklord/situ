...
class Emitter:
	...
	def _tag(self, struct: ResolvedStruct, placement: Placement) -> list[str]:
		...
		if run is not None:
			...
			lines.extend([
				...
				f"\t\tlet start = {self._region_start(struct, first)};",
	...
	def _unimplemented_codecs(self) -> frozenset[str]:
		...
		compiler rather than being declined. Rust inlines the
	...
	def _derived_codecs(self) -> list[str]:
		...
		for decl in self.schema.codecs():
			...
			if body is None:
				# What `derived.generate` says on the same condition. It is
				# not "wellformed admits only derived codecs", which is what
				# the assertion here claimed: `crc7_mmc` IS derived, and
				# what was declined is its WIDTH -- a non-reflected code
				# narrower than a byte needs a left-aligned loop that no
				# backend writes yet (0046, 26.368).
				assert decl.kernel is not None
				lines.extend([
					"",
					f"// No implementation for `{decl.name}`: a "
					f"{decl.kernel.family.value} kernel is described",
					"// but not yet generated in Rust. Its properties are",
	...
	def _region_start(self, struct: ResolvedStruct,
		...
		if inside:
			...
			return self._unparen(found) if found is not None else '0'
	...
	def _decode_accessor(self, struct: ResolvedStruct, placement: Placement,
		...
		return [
			...
			*(
			  # A BOUNDED ratio yields 1:1 from `decode_ratio`, which is the
			  # only safe upper bound -- but a stuffing code does not
			  # preserve length, so saying it does would replace the wrong
			  # arithmetic with a wrong sentence.
			  [f"\t/// parameter -- {placement.codec} stuffs bytes in, so"
			   " decoding takes",
			   "\t/// them out and never puts more back. The value is at"
			   " most as long",
			   "\t/// as the wire form, and that is the buffer."]
	...
	def element_value(self, struct: ResolvedStruct, run: Placement,
		...
		at = f"({base}) + {fixed}"
		return f"({self._load(member, member.scalar, at)} as i64)"
