...
| [dnsname](dnsname/) | The boundary, in one file: situ describes a compressed name completely and cannot follow the pointer, and the difference between those is the view model rather than a gap. Also the one weakening a schema has to declare because the layout does not imply it. |
