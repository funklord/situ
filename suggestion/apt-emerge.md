...
```
$ situc build example/http/http.situ --target python --out /tmp/gen
...
with space-continued lines. That is almost exactly `example/http/http.situ`:
...
`gen-fuzz` exist, and a large share of `example/` are formats with an
