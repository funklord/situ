...
**Where tag verification's timing lives, stated because a consumer asked
(`suggestion/fuzznet.md`) and confirmed by reading the emitted code rather
than the intent.** The generated code never computes or compares an
authentication tag. A sealed region's `_open()` takes the caller's verdict
as a parameter -- `open(view, int verified, ...)` -- and refuses on the
flag, so the comparison that decides `verified`, and whether it is constant
time, belongs wholly to the caller and the codec they chose. What situ
generates for a tag or checksum is the geometry: the covered span, the
field's own bytes, `self_as`. Its own branches are over structure --
lengths, discriminants, the verified flag -- never over tag bytes, and the
one `== tag` in any backend compares a TLV wire-tag *number*, not a MAC.
The division of labour is therefore exactly the safe reading: **the
primitive and its constant-time comparison are the caller's; the bytes it
runs over are the schema's.** A future generated comparison would be a
decision record, not a drive-by (13.1 already forbids situ knowing the
algorithm).
...
### 21.1 The generator version is a contract, stated where it binds

An adopter commits the generated sources and treats situc as a build-time
tool, which raises the question every serious one reaches
(`suggestion/netcfgd.md`): when the sources are regenerated, what promises
that the same schema means the same bytes?

Three parts, the first now mechanical:

- **Every generated file names its generator.** The banner reads
  `Generated by situc 1.0 from x.situ`, with the version from the same
  `VERSION` file the package and `situc --version` read. A regeneration
  under a different situc is therefore a visible diff line, not a silent
  substitution -- the reviewer sees the generator moved even when nothing
  else did.
- **The wire signature is the compatibility oracle, not the generator
  version.** Two situc versions may emit different code for one schema;
  what must not change unnoticed is what the bytes mean, and that is what
  the committed `.situ.wire` diff answers. `situc diff` compares
  capability; the wire file compares contract. A generator upgrade that
  moves neither is a refactor of the emitted text.
- **Output stability is promised per wire signature, not per byte of
  code.** The emitted text may improve between versions; the committed
  copy pins what a given tree builds against, and regenerating is a
  reviewed change like any other. A guarantee of byte-stable output across
  versions would freeze every emitter bug, and nobody is served by that.
...
An array of wide scalars went in as well -- an element wider than a byte is
`ValueConverted`, so it is reached by index rather than by pointer, and Rust
answers with a `Result` where C and C++ return the value. The first element is
always there, the count being the schema's rather than the message's, so the
shapes agree about it and the probe needs no bounds question.
...
the language does not support the construct, where the language supports
...
time; in four of those the note was the *better* half of the problem, C having
answered on its own and answered wrongly. Section 0's rule applied to section
26.31: prefer adding a check to adding a promise. Its exemption list is empty,
which is the point -- it was not empty when it was written.
...
to diff. The underlying integer would compare and no backend exposes it, which
...
### 26.131 Three answers to the consumers

The `suggestion/` directory is six files of evaluations written from the
other private projects, and it turned out to be the backlog: concrete asks
with reproductions attached, from the sessions that actually tried adoption.
Three are answered; the answers went back into the files that asked, because
the directory is correspondence rather than an archive.

**fuzznet asked for one sentence and got a normative paragraph.** Their
traffic reconfigures infrastructure across untrusted networks, and they
wanted the timing division confirmed: who owns the constant-time comparison?
Measured rather than asserted: the generated code never computes or compares
a tag anywhere -- `_open()` takes the caller's *verdict* as a parameter, and
the one `== tag` in any backend compares a TLV wire-tag number. 14.6 now
states the division: the primitive and its constant-time comparison are the
caller's; the bytes it runs over are the schema's; a future generated
comparison would be a decision record, not a drive-by.

**netcfgd's version question became 21.1 and a banner.** Every generated
file now names its generator -- `Generated by situc 1.0 from x.situ`, from
the same `VERSION` the package and `situc --version` read -- so a
regeneration under a different situc is a visible diff line rather than a
silent substitution. The policy beside it: the committed `.situ.wire` is the
compatibility oracle, not the generator version, and byte-stable emitted
code across versions is deliberately not promised, because it would freeze
every emitter bug. The sweep that stamped 33 files also produced this
entry's incident: the script inserted the version import "before the first
import line, which is always safe" -- and `from __future__` must be first,
so four files stopped parsing and 139 CLI tests went red at once. An
insertion rule that is safe in most files is a rule, not a fact, and the
suite said so immediately.

**netcfgd's flagship ask is `situc gen-tamper`.** They adopted for the
verify gate and hold that a gate nobody has watched fail is not evidence;
the generated harness is the watching. It takes the caller's verifier as a
callback -- the division of 14.6, load-bearing within a week of being
written -- and drives it across the schema's own coverage geometry through
the same generated `_covered` helpers the caller's recompute uses: every
covered byte and every tag byte flipped one at a time with refusal required,
and, for a fixed layout, every outside byte flipped with the answer required
not to change, which is what catches a verifier covering more than the
schema says. The outside half is deliberately absent for dynamic layouts --
a flipped byte can be a length there, and a parse that legitimately moved is
not an over-covering verifier.

The executable test is its own control pair: an honest XOR verifier survives
every flip, and a deliberately lying one that ignores the last covered byte
is caught with `failed_at` naming that byte's exact offset. A harness only
the honest half exercised could pass vacuously; the liar is what proves the
