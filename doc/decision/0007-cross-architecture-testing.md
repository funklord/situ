...
Closes the revisit that `doc/decision/0004-aarch64-compile-only.md` deferred
to this phase.
...
`make cross-test` runs `test/cross`, which does three things:
...
- `test/cross` is a self-contained sub-project like the others, with its own
